...
For evaluating a single system, \scorio{} uses outcome matrices \(E \in \{0,\ldots,C\}^{M\times N}\), where \(M\) is the number of tasks, \(N\) the number of trials, and \(C\) the maximum category label (\(C+1\) outcome categories in total). For ranking multiple systems, it uses response tensors \(R \in \{0,\ldots,C\}^{L\times M\times N}\), where \(L\) is the number of systems. Two-dimensional inputs are promoted automatically to a single-trial tensor, so one-shot benchmarks use the same code path.

Different method families consume different views of \(R\). Pointwise methods work with trial means \(\bar r_{\ell m} = \frac{1}{N}\sum_{n=1}^{N} R_{\ell mn}\) or rubric-weighted scores. Pairwise methods work with induced win/tie counts \(W_{\ell,\ell'} = \lvert\{(m,n) : R_{\ell mn} > R_{\ell' mn}\}\rvert\). Voting and listwise methods use the per-task ordering \((R_{1mn},\ldots,R_{Lmn})\). Graph methods build a weighted comparison graph. IRT methods estimate latent abilities and task difficulties from the same binary or graded outcomes. All these conversions happen inside the package; the user always passes the same tensor.
...
We test three questions: (i)~whether representative methods recover a known synthetic ranking, (ii)~how stable the rankings are when only a few trials are available, and (iii)~what the practical runtime costs look like. The results below come from the \texttt{pilot} profile. Figure~\ref{fig:additional-pilot} provides complementary views through mean absolute rank error and top-1 stability.
